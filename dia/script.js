...
function setupEventListener() {
  ...
  async function callRequirementAnalysisAPI(userBrief) {
    if (!userBrief.trim()) {
        alert('Please provide a valid brief to analyze.');
        return;
    }

    try {
        const response = await fetch('https://saya.unbelong.in/api/requirement-analysis', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        console.error('Error calling Requirement Analysis API:', error);
        alert('Failed to analyze requirements. Check console for details.');
    }
}
  ...
  document.getElementById('doc-content').addEventListener('click', async (e) => {
    console.log("Something is cliked");
    ...
    if (e.target.classList.contains("runflowButton")) {
      console.log("Run Flow is clicked");
      // Get the parent bullet-point div
      const bulletPointDiv = e.target.closest('.bullet-point');

      // Find the title element within this div
      const titleElement = bulletPointDiv.querySelector('.title');
      let nodeId = titleElement.dataset.id
      // Get the content of the title element
      const titleValue = titleElement.textContent.trim();
      let discoverRespose = await callRequirementAnalysisAPI(titleValue)
      let node = await tree.createNode(discoverRespose,nodeId)
      tree.insertOneBulletPoint(node, bulletPointDiv)
      // orchestrator.runFlow(titleValue, nodeId)
